...
from tree import UserBehaviorTree
...
def generate_and_obtain_dataset(
    ...
    path_ = Path(
        log_dir + f"/dataset/{reward_type}_{reward_structure_type}_{decay_function_}"
    )
    ...
    bandit_feedback = dataset.obtain_batch_bandit_feedback(
        n_rounds, return_basic_pscore=True
    )
    ...
    n_samples = len(bandit_feedback["pscore"])
    ubtree_candidate_reward_structures = obtain_candidate_reward_structures(
        len_list=len_list,
        random_state=base_random_state,
    )[0]
    n_candidate_reward_structures = len(ubtree_candidate_reward_structures)
    pscore_all = np.zeros((n_candidate_reward_structures, n_samples))
    evaluation_policy_pscore_all = np.zeros((n_candidate_reward_structures, n_samples))
    for i, reward_structure in enumerate(ubtree_candidate_reward_structures):
        pscore_all[i] = dataset.calc_pscore_given_reward_structure(
            policy_logit_=behavior_policy_logit_,
            action=bandit_feedback["action"],
            reward_structure=np.tile(reward_structure, (n_samples // len_list, 1, 1)),
        )
        evaluation_policy_pscore_all[i] = dataset.calc_pscore_given_reward_structure(
            policy_logit_=evaluation_policy_logit_,
            action=bandit_feedback["action"],
            reward_structure=np.tile(reward_structure, (n_samples // len_list, 1, 1)),
            epsilon=epsilon,
        )
    bandit_feedback["pscore_all"] = pscore_all
    bandit_feedback["evaluation_policy_pscore_all"] = evaluation_policy_pscore_all
...
def evaluate_estimators(
    ...
    **kwargs,
    ...
    aips = SlateAdaptiveIPS(len_list=len_list, estimator_name="AIPS")
    # script
    ...
    ope = SlateOffPolicyEvaluation(
        ...
        ope_estimators=[ips, iips, rips, aips_true],
    )
    ...
    # gips w/ optimized user behavior tree
    ubtree = UserBehaviorTree(
        dataset=dataset,
        candidate_models=obtain_candidate_reward_structures(
            len_list=len_list,
            random_state=base_random_state,
        )[0],
        behavior_policy_function=behavior_policy_function,
        evaluation_policy_beta=evaluation_policy_beta,
        evaluation_policy_epsilon=epsilon,
        len_list=len_list,
        random_state=random_state,
        **ubtree_hyperparams,
    )
    optimized_user_behavior_model = np.zeros((n_samples, len_list), dtype=int)
    bootstrap_dataset = [
        dataset.obtain_batch_bandit_feedback(n_samples)
        for _ in range(ubtree.n_bootstrap)
    ]
    for position in tqdm(
        np.arange(len_list),
        desc="[user_behavior_tuning]",
        total=len_list,
    ):
        optimized_user_behavior_model[:, position] = ubtree.fit_predict(
            context=bandit_feedback["context"],
            behavior_policy_pscore=bandit_feedback["pscore_all"],
            evaluation_policy_pscore=bandit_feedback["evaluation_policy_pscore_all"],
            reward=bandit_feedback["reward"],
            position=position,
            bootstrap_dataset=bootstrap_dataset,
        )
    optimized_user_behavior_model = optimized_user_behavior_model.flatten()
    ope_aips = SlateOffPolicyEvaluation(
        bandit_feedback=bandit_feedback,
        ope_estimators=[aips],
    )
    behavior_policy_pscore_given_user_behavior_model = (
        ope_aips.calc_pscore_given_user_behavior(
            policy_pscore_all=bandit_feedback["pscore_all"],
            user_behavior_model=optimized_user_behavior_model,
        )
    )
    evaluation_policy_pscore_given_user_behavior_model = (
        ope_aips.calc_pscore_given_user_behavior(
            policy_pscore_all=bandit_feedback["evaluation_policy_pscore_all"],
            user_behavior_model=optimized_user_behavior_model,
        )
    )
    estimation_dict_aips_ = ope_aips.estimate_policy_values(
        evaluation_policy_pscore=bandit_feedback["evaluation_policy_pscore"],
        evaluation_policy_pscore_item_position=bandit_feedback[
            "evaluation_policy_pscore_item_position"
        ],
    ...
    estimation_dict_["AIPS"] = estimation_dict_aips_["AIPS"]
...
def process(
    ...
    for return_ in returns:
        estimation_dict, target_value, random_state = return_.get()

        for estimation_dict_, target_value_ in zip(estimation_dict, target_value):
            for estimator_name in estimators_name:
                estimation_ = estimation_dict_[estimator_name]
    ...
    df = pd.DataFrame()
    df["IPS"] = estimators_performance["IPS"]
    df["IIPS"] = estimators_performance["IIPS"]
    df["RIPS"] = estimators_performance["RIPS"]
    df["AIPS"] = estimators_performance["AIPS"]
    df["AIPS (true)"] = estimators_performance["AIPS (true)"]
    df["on_policy"] = estimators_performance["on_policy"]
    df["ground_truth"] = estimators_performance["ground_truth"]
    df["target_value"] = target_values
    df["random_state"] = random_states
    df.to_csv(f"estimation_{experiment}.csv", index=False)
...
def wrapper_evaluate_estimators(args):
    ...
    if target_param in ["n_rounds", "len_list"]:
        for i, value in enumerate(conf[target_param]):
            ...
            estimation_dict.append(evaluate_estimators(**conf_))
    ...
    return estimation_dict, target_value, random_state
